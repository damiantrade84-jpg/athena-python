...
_ENGINE_B_AI_EXPERT_PREFIX_FALLBACK = (
    ...
    "For scale-out plans: compare RR1 to Engine B TP1 minimum RR and RR2 / rrUsedForGate to "
    "`Style min RR (config)` from AI CALIBRATION CONTEXT only — do NOT compare RR1 to style min RR "
    "when scaleOutActive=true and RR1 passes tp1MinRr with tp1PathClear=true. "
    "spaceGateOk is the authoritative deterministic room gate; roomOk=false alone is not an automatic reject "
    "when spaceGateOk=true via approved geometric substitution or scale-out. "
    "A TP1 that would overshoot the opposing zone is clamped to the wall's front edge "
    "(tp1ClampedToOpposingZone=true) — the emitted TP1 is reachable; do not reject it for the pre-clamp overshoot. "
    "Distinguish structural invalidation SL from ATR/mechanical execution SL: "
    "executionSlTighterThanStructural=true is the normal design, not a levels defect. "
