    ...
    updated.update(
        {
            ...
            "quality_pct": refreshed.get("quality_pct"),
        ...
        try:
            ...
            for direction in ["LONG", "SHORT"]:
                ...
                signal = {
                    ...
                    "quality_pct": conf_data.get("quality_pct"),
                    "is_naked": True,
...
def api_feature_toggles():
    ...
    elif feature == "profitability_gates":
        current = bool(CONFIG.get("ENGINE_AB_PROFITABILITY_GATES_ENFORCED", False))
        new_val = not current if action == "toggle" else (action == "enable")
        CONFIG["ENGINE_AB_PROFITABILITY_GATES_ENFORCED"] = new_val
        # Keep legacy knobs synchronized so enabling this one UI switch restores
        # every removed Engine A/B RR/max-SL block.
        CONFIG["ENGINE_A_RR_GATE_ENABLED"] = new_val
        CONFIG["ENGINE_B_RR_GATE_ENABLED"] = new_val
        CONFIG["ENGINE_B_ENFORCE_MAX_SL_PCT"] = new_val
        log.warning(
...
def api_chart_analysis():
    ...
    try:
        ...
        log.info(
            ...
            f"model={get_ai_model(CONFIG, 'VISION_MODEL', 'grok-4.6', provider=_active_provider)}"
        )
