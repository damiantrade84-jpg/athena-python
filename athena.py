...
def api_chart_analysis():
    ...
    system_prompt = (
        ...
        "8. ENTRY POSITIONING: Assess whether the entry is at a tactical location (retracement to S/R, pullback to EMA, zone re-test) "
        ...
        "9. VOLATILITY-REGIME INTERACTION: If the algorithmic regime is LOW_VOLATILITY or DEAD_RANGING, assess whether the setup suits that environment. "
        ...
        "10. MOVE MATURITY: Estimate where in the trend leg the entry sits. If the trend has already moved significantly "
        "and the TP target is a small fraction of the move already completed, flag the entry as potentially late/exhausted.\n"
        "11. RR REALITY CHECK: Independently verify the risk-reward by comparing visible SL distance vs TP distance from entry. "
    ...
    user_prompt = (
        ...
        "describe what you see in the label area (e.g. 'top-left shows a dark bar with no readable text') and then fall back to "
        "algorithmic context from STEP 2 for the instrument name. State 'chart label not legible — using algo context'.\n"
        ...
        "Only fall back to algorithmic context if genuinely unreadable after checking all label areas.\n"
    ...
    try:
        ...
        if triple_mode:
            ...
            triple_prompt = (
                ...
                "Only if genuinely unreadable after checking all areas, state what you see in the label area and fall back to algorithmic context from STEP 2.\n"
                ...
                "ACTIONABLE IMPROVEMENT\n\n"
                "Instructions:\n"
                "- TRADE SNAPSHOT: one line for instrument + each TF (D1/H4/H1); read from the chart image header bar. Only fall back to algo context if genuinely unreadable.\n"
