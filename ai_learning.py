"""ai_learning.py — AI self-learning and outcome tracking for Athena Pro.
...
def extract_learning_from_trade(db_path: str, ticket: str, is_demo: bool = False) -> None:
...
def _infer_asset_type(pair: str) -> str:
    ...
    if any(c in p for c in ["XAU", "XAG", "OIL"]): return "commodity"
    if any(c in p for c in ["EUR","GBP","USD","JPY","AUD","NZD","CHF","CAD"]): return "forex"
    return "stock"


# ── Learning context query ────────────────────────────────────────────────────

def get_ai_learning_context(pair: str, asset_type: str, db_path: str,
    ...
    if len(pair_rows) >= 3:
        ...
        regimes = [r.get("regime") for r in pair_rows if r.get("win") and r.get("regime")]
...
Based on the above outcomes data, provide:
1. The 2-3 most significant patterns (what's working, what's not)
2. Specific adjustments to make (e.g. "reduce position size on B-grade crypto in RANGING regimes")
3. One thing the signal system is systematically getting wrong

Reply in JSON only:
{{"summary":"one sentence overall assessment","insights":["insight1","insight2","insight3"],"adjustments":["adj1","adj2"],"blind_spot":"one key systematic miss"}}"""
