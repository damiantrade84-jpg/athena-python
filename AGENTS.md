...
Do not intentionally load `CLAUDE.md`. Claude Code uses `CLAUDE.md`.
...
Karpathy-inspired coding behavior is applied automatically in Cursor via `.cursor/rules/karpathy-guidelines.mdc`. Claude Code uses the same principles in `CLAUDE.md` (keep both in sync when updating upstream).
