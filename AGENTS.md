...
- Auto execution must honor `EXECUTION_ENABLED`, broker execution switches, kill switch, risk engine, and guardian gates.
- AI debate/chart review may block, wait, or reduce `executionConvictionEffective`; it must never mutate Engine A score, threshold, pass/fail, or score attribution.
- Browser chart metadata is diagnostic only and must not affect scoring or execution permission.
