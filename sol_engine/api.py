"""Flask routes for the SOL dashboard and execution workflow."""
...
from .execution import SolExecutionError
from .service import ScanAlreadyRunning, SolService
...
def register_sol_routes(app, runtime) -> SolService:
    service: SolService = runtime.service
    ...
    def preview(signal_id: str):
        try:
            ...
            # A rejected preview is still a successfully evaluated resource;
            # return its gate evidence so the UI can explain the rejection.
    ...
    add("/api/sol/signals", "sol_signals", signals)
    add("/api/sol/signals/<signal_id>", "sol_signal_detail", signal_detail)
    add("/api/sol/signals/<signal_id>/preview", "sol_signal_preview", preview, ("POST",))
    add("/api/sol/signals/<signal_id>/execute", "sol_signal_execute", execute, ("POST",))
    add("/api/sol/executions", "sol_executions", executions)
    add("/api/sol/replay", "sol_replay", replay, ("POST",))
    return service
