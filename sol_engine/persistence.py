"""SQLite persistence and idempotency ledger for SOL."""
...
class SolRepository:
    ...
    def migrate(self) -> None:
        with self._migration_lock, self._connect() as con:
            con.executescript(
                """
                CREATE TABLE IF NOT EXISTS sol_scans (
                ...
                CREATE TABLE IF NOT EXISTS sol_signals (
                    ...
                    FOREIGN KEY(scan_id) REFERENCES sol_scans(scan_id)
                );
                CREATE INDEX IF NOT EXISTS idx_sol_signals_scan_score
                    ON sol_signals(scan_id, decision, score DESC);

                CREATE TABLE IF NOT EXISTS sol_executions (
                    ...
                    FOREIGN KEY(signal_id) REFERENCES sol_signals(signal_id)
                );
                CREATE INDEX IF NOT EXISTS idx_sol_executions_signal
                    ON sol_executions(signal_id, requested_at DESC);
                """
            )

    def create_scan(self, request_payload: dict[str, Any]) -> str:
        scan_id = "solscan_" + uuid.uuid4().hex
        with self._connect() as con:
            con.execute(
                "INSERT INTO sol_scans(scan_id,started_at,status,request_json,summary_json) VALUES(?,?,?,?,?)",
    ...
    def complete_scan(self, scan_id: str, status: str, summary: dict[str, Any]) -> None:
        with self._connect() as con:
            con.execute(
                "UPDATE sol_scans SET completed_at=?, status=?, summary_json=? WHERE scan_id=?",
    ...
    def save_signals(self, scan_id: str, signals: list[dict[str, Any]]) -> None:
        ...
        with self._connect() as con:
            con.executemany(
                """
                INSERT INTO sol_signals(
                    signal_id,scan_id,pair,asset_type,decision,score,generated_at,payload_json
                ) VALUES(?,?,?,?,?,?,?,?)
    ...
    def latest_scan(self) -> dict[str, Any] | None:
        with self._connect() as con:
            row = con.execute("SELECT * FROM sol_scans ORDER BY started_at DESC LIMIT 1").fetchone()
        return self._scan_dict(row) if row else None

    def get_scan(self, scan_id: str) -> dict[str, Any] | None:
        with self._connect() as con:
            row = con.execute("SELECT * FROM sol_scans WHERE scan_id=?", (scan_id,)).fetchone()
        return self._scan_dict(row) if row else None
    ...
    def latest_completed_scan_id(self) -> str | None:
        with self._connect() as con:
            row = con.execute(
                "SELECT scan_id FROM sol_scans WHERE status='COMPLETED' ORDER BY completed_at DESC LIMIT 1"
            ).fetchone()
    ...
    def list_signals(
        ...
        *,
        ...
        query = f"SELECT payload_json FROM sol_signals WHERE {' AND '.join(clauses)} ORDER BY score DESC, pair ASC LIMIT ?"
        with self._connect() as con:
    ...
    def get_signal(self, signal_id: str) -> dict[str, Any] | None:
        with self._connect() as con:
            row = con.execute("SELECT payload_json FROM sol_signals WHERE signal_id=?", (signal_id,)).fetchone()
    ...
    def reserve_execution(
        ...
        *,
        ...
        execution_id = "solexec_" + uuid.uuid4().hex
        with self._connect() as con:
            con.execute("BEGIN IMMEDIATE")
            existing_key = con.execute(
                "SELECT * FROM sol_executions WHERE idempotency_key=?", (idempotency_key,)
            ).fetchone()
            if existing_key:
                return self._execution_dict(existing_key), False
            active = con.execute(
                """
                SELECT * FROM sol_executions
            ...
            row = con.execute("SELECT * FROM sol_executions WHERE execution_id=?", (execution_id,)).fetchone()
    ...
    def complete_execution(self, execution_id: str, status: str, result: dict[str, Any]) -> dict[str, Any]:
        with self._connect() as con:
            con.execute(
                "UPDATE sol_executions SET status=?, completed_at=?, result_json=? WHERE execution_id=?",
    ...
    def list_executions(self, *, limit: int = 100) -> list[dict[str, Any]]:
        with self._connect() as con:
            rows = con.execute(
                "SELECT * FROM sol_executions ORDER BY requested_at DESC LIMIT ?",
