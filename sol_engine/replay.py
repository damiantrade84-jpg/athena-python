"""Causal point-in-time replay for SOL research diagnostics."""
...
def _outcome(signal: dict[str, Any], future: list[Candle]) -> dict[str, Any]:
    ...
    for index, candle in enumerate(future, 1):
        ...
        # Ambiguous same-bar ordering is resolved pessimistically.
        if stop_hit:
...
def run_causal_replay(
    *,
    ...
    """Replay prefixes only; the evaluator never sees a candle after ``as_of``."""
        ...
        trades.append(
            {
                ...
                "barClosedAt": signal["barClosedAt"],
    ...
    r_values = [float(row["rMultiple"]) for row in trades]
    wins = sum(1 for row in trades if float(row["rMultiple"]) > 0)
    losses = sum(1 for row in trades if float(row["rMultiple"]) < 0)
    equity = 0.0
    peak = 0.0
    max_drawdown = 0.0
    ...
    sample_minimum = int(config.replay["minimum_trades_for_evidence"])
    return {
        "contractVersion": config.version,
        "pair": str(pair.get("display") or pair.get("symbol") or "UNKNOWN"),
        "generatedAt": utc_iso(m5[-1].closes_at("M5")) if m5 else None,
        "causal": True,
        ...
        "metrics": {
            ...
            "wins": wins,
            "losses": losses,
            "winRatePct": round(wins / len(trades) * 100.0, 2) if trades else None,
            "averageR": round(sum(r_values) / len(r_values), 4) if r_values else None,
            "totalR": round(sum(r_values), 4),
            "maxDrawdownR": round(max_drawdown, 4),
        },
        "evidenceStatus": "SAMPLE_SUFFICIENT" if len(trades) >= sample_minimum else "INSUFFICIENT_SAMPLE",
        "minimumTradesForEvidence": sample_minimum,
    }
