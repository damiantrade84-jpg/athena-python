"""Institutional session clock for SOL (New York time, ICT windows)."""
...
def _display_zone(config: SolConfig):
...
def ny_datetime(epoch: float, config: SolConfig) -> datetime:
    return datetime.fromtimestamp(float(epoch), tz=timezone.utc).astimezone(_zone(config))


def display_datetime(epoch: float, config: SolConfig) -> datetime:
...
def market_is_open(epoch: float, config: SolConfig, asset_type: str) -> bool:
...
def classify_session(epoch: float, config: SolConfig) -> dict[str, Any]:
    local = ny_datetime(epoch, config)
...
def session_bounds(epoch: float, config: SolConfig, name: str) -> tuple[float, float] | None:
    windows = config.sessions["windows"]
...
def prior_day_envelope(candles: list[Candle], epoch: float, config: SolConfig) -> dict[str, Any]:
    if not candles:
        return {"available": False, "reason": "NO_BARS"}
    local = ny_datetime(epoch, config)
    today_start = local.replace(hour=0, minute=0, second=0, microsecond=0)
    yesterday_start = today_start - timedelta(days=1)
    start = yesterday_start.timestamp()
    end = today_start.timestamp()
    rows = [candle for candle in candles if start <= candle.time < end]
    if not rows:
        return {"available": False, "reason": "NO_PRIOR_DAY_BARS"}
    return {
        "available": True,
        "high": max(candle.high for candle in rows),
        "low": min(candle.low for candle in rows),
...
def session_pools(candles: list[Candle], epoch: float, config: SolConfig) -> dict[str, Any]:
    asia = envelope_for_window(candles, epoch, config, "asia_range")
    prior = prior_day_envelope(candles, epoch, config)
    return {
        "asia": asia,
        "asiaHigh": asia.get("high"),
        "asiaLow": asia.get("low"),
        "asiaAvailable": bool(asia.get("available")),
        "priorDayHigh": prior.get("high"),
        "priorDayLow": prior.get("low"),
        "priorDayAvailable": bool(prior.get("available")),
    }
