"""Pure helpers for Binance crypto WebSocket / microstructure subscription scope."""
...
def enabled_binance_micro_crypto_pairs(crypto_pairs: Optional[list[dict[str, Any]]]) -> list[dict[str, Any]]:
    ...
    Rules: enabled (default True), type crypto, source binance, non-empty futures symbol."""
    if not crypto_pairs:
        return []
