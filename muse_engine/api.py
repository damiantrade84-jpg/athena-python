"""Flask routes for the MUSE command deck and execution workflow."""
...
from .execution import MuseExecutionError
from .service import MuseService, ScanAlreadyRunning
...
def register_muse_routes(app, runtime) -> MuseService:
    service: MuseService = runtime.service
    ...
    def scan():
        ...
        asset_types = {str(v).strip().lower() for v in asset_types_raw or [] if str(v).strip()}
        symbols = {str(v).strip() for v in symbols_raw or [] if str(v).strip()}
        try:
    ...
    def signals():
        ...
        if decisions:
            decisions = {v.upper() for v in decisions}
            if not decisions.issubset({"PRIME", "STAGE", "DORMANT", "BLOCKED"}):
    ...
    def execute(signal_id: str):
        ...
        try:
            result = service.execute_signal(signal_id, mode=mode, idempotency_key=key,
        ...
        return jsonify({**result, "success": False, "error": str(code)}), (403 if status == "REJECTED" else 503)
    ...
    def sounding():
        ...
        try:
            ...
            result = service.sounding_pair(symbol, bars=bars)
            return jsonify({"success": True, **result})
        except (TypeError, ValueError):
            ...
            return _error("sounding_failed", 503, detail=str(exc))

    add("/api/muse/health", "muse_health", health)
    add("/api/muse/config", "muse_config", config)
    add("/api/muse/accounts", "muse_accounts", accounts)
    add("/api/muse/scan", "muse_scan", scan, ("POST",))
    add("/api/muse/scan/current", "muse_scan_current", current_scan)
    add("/api/muse/signals", "muse_signals", signals)
    add("/api/muse/signals/<signal_id>", "muse_signal_detail", signal_detail)
    add("/api/muse/signals/<signal_id>/preview", "muse_signal_preview", preview, ("POST",))
    add("/api/muse/signals/<signal_id>/execute", "muse_signal_execute", execute, ("POST",))
    add("/api/muse/executions", "muse_executions", executions)
    add("/api/muse/sounding", "muse_sounding", sounding, ("POST",))
    return service
