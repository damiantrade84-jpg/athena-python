...
- **Entry quality rules (2026-04-01):** System prompt rules 8–11 enforce entry positioning assessment (tactical vs chasing into congestion), volatility-regime interaction (low-vol breakdowns flagged), move maturity (late/exhausted entries), and independent RR verification. All three user prompts (single/dual/triple) include an **ENTRY QUALITY** body section between **KEY RISKS** and **FINAL VERDICT**.
...
28. **Vision ENTRY QUALITY section:** Must appear between **KEY RISKS** and **FINAL VERDICT** in all three prompt modes (single/dual/triple). System prompt rules **8–11** (entry positioning, volatility-regime interaction, move maturity, RR reality check) are mandatory — do not remove or weaken.
