...
def test_bt_indicators_from_cache_passes_score_group(monkeypatch):
    import backtest_runner

    captured = []

    def _fake_calc(candles, asset_type, score_group=None):
        captured.append({"n": len(candles), "asset_type": asset_type, "score_group": score_group})
        return {"snap": {}, "score_group": score_group}

    monkeypatch.setattr(backtest_runner, "calc_indicators_with_normalized", _fake_calc)

    candles = [{"close": float(i)} for i in range(60)]
    cache: dict = {}

    out = backtest_runner._bt_indicators_from_cache(
        candles,
        end_idx=59,
        window=60,
        asset_type="forex",
    ...
    assert out["score_group"] == "forex_majors"
    assert captured[-1]["score_group"] == "forex_majors"

    # Different score_group must not collide in the cache.
    out2 = backtest_runner._bt_indicators_from_cache(
        candles,
        end_idx=59,
        window=60,
        asset_type="forex",
    ...
    assert out2["score_group"] == "forex_crosses"
    assert len(cache) == 2

    # Cache hit: same group returns cached result without recompute.
    calls_before = len(captured)
    out3 = backtest_runner._bt_indicators_from_cache(
        candles,
        end_idx=59,
        window=60,
        asset_type="forex",
...
def test_backtest_engine_a_indicator_calls_pass_score_group_in_source():
    """Source pin: every Engine A backtest indicator call must pass score_group
    (live analyze_pair passes it; omitting it reverts to universal periods)."""

    def _call_windows(body: str, name: str) -> list[str]:
        windows = []
        search_from = 0
        while True:
            ...
            if pos == -1:
                break
            # Multi-line calls: a 300-char window covers all arguments.
            windows.append(body[pos : pos + 300])
            search_from = pos + 1
        return windows

    src = open(os.path.join(_REPO_ROOT, "backtest_runner.py"), encoding="utf-8").read()
    start = src.index("def backtest_pair(")
    end = src.index("def _format_backtest_results(")
    body = src[start:end]

    direct_calls = _call_windows(body, "calc_indicators_with_normalized")
    assert direct_calls, "expected Engine A indicator calls in backtest_pair"
    for call in direct_calls:
        assert "score_group=_pair_score_group" in call, f"missing score_group: {call}"

    cached_calls = _call_windows(body, "_bt_indicators_from_cache")
    assert cached_calls, "expected vectorized Engine A indicator calls in backtest_pair"
