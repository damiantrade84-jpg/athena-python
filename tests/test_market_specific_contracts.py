...
def test_athena_source_uses_class_specific_engine_b_overlay_gate():
    """Engine B overlay is gated by resolved scan score vs get_score_threshold (class/group routing)."""
    src = ATHENA_PATH.read_text(encoding="utf-8")
    assert "get_score_threshold(pair, regime=_engine_a_threshold_regime)" in src
