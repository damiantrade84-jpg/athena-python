...
def test_dashboard_and_frequency_pipeline():
    ...
    assert len(board["number_frequency"]) == 58  # LOTTERY_GAME_RULES.lotto main_min..main_max
    assert board["recommended_sum_range"]["min"] < board["recommended_sum_range"]["max"]
    assert len(board["positional_distribution"]) == 6
    assert len(board["rolling_frequency"]) == 58
...
def test_generator_scoring_and_simulation():
    ...
    # simulate_generator skips ticket gen on draw 0 (seed counters only) — 9 draws × 3 tickets
    assert sim["tickets_generated"] == 27
...
def test_new_lottery_analytics_and_wheel():
    ...
    assert sum_range["p_low"] < sum_range["p_high"]
