...
def ase_scan_signal_for_pair(
    pair: dict[str, Any],
    *,
    style: str = "swing",
    ptis_root: str | None = None,
) -> dict[str, Any] | None:
    """Single-pair ASE signal for scanner legacy-bypass fallback."""
    symbol = str(pair.get("symbol") or pair.get("display") or "").replace("/", "")
    inst = instrument_by_symbol(symbol)
    if inst is None:
        return None
    horizon: Horizon = "intraday" if str(style).lower() == "intraday" else "swing"
    store = PTISStore(ptis_root or default_ptis_root())
    candidates = generate_live_candidates(store, (inst,), horizon=horizon)
    if candidates:
        signals = predict_batch(candidates, store, {inst.symbol: inst})
        if signals:
            return signals[0].to_engine_a_dict(style=style)
    flat = predict_no_candidate(inst, horizon, store=store)
    return flat.to_engine_a_dict(style=style)
