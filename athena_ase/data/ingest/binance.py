"""Ingest Binance Futures OHLCV from Athena's backtest candle cache into PTIS."""
...
log = logging.getLogger("ase.ingest.binance")
    ...
    try:
        with sqlite3.connect(_db_path(db_path), timeout=15.0) as con:
            ...
            rows = con.execute(
                ...
                WHERE provider = ? AND asset_type = 'crypto'
                ORDER BY symbol
                """,
                (_PROVIDER,),
            ).fetchall()
    ...
    with sqlite3.connect(path, timeout=15.0) as con:
        ...
        rows = con.execute(
            ...
            WHERE display IS ? AND symbol IS ? AND source IS ?
              AND asset_type IS ? AND timeframe = ? AND provider = ?
            ORDER BY bar_time
            """,
            (
                pair.get("display"),
    ...
    *,
    ...
    for bar in bars:
        ...
        for field in _FIELDS:
            ...
            value = float(raw)
            sid = price_series_id(_SOURCE, symbol, tf, field)
    ...
    for field, rows in field_rows.items():
        sid = price_series_id(_SOURCE, symbol, tf, field)
        counts[sid] = append_ptis_rows(store, sid, _SOURCE, rows)
    return counts
    ...
    for pair in list_cached_pairs(db_path=db_path):
        for tf in timeframes:
            try:
                ...
                log.warning(
                    "Binance ingest failed %s %s: %s",
                    pair.get("display"),
                    tf,
                    exc,
                )
    log.info("Binance ingest complete: %d series touched", len(totals))
    return totals
