...
if is_naked:
    if not target_pair:
        print(
            "Error: Must specify a pair for --naked backtest (e.g., python run_backtest.py EUR/USD --naked)"
        )
        sys.exit(1)

    print(f"=== Engine B (Naked) Backtest for {target_pair} ===")
    r = requests.post(
        "http://localhost:5000/api/backtest-naked",
        json={"pair": target_pair},
        timeout=300,
    )
    ...
    print("-" * 75)
    print(
        f"{d.get('pair', target_pair):15s} {d.get('sqn', 0):+7.2f} {d.get('winRate', 0):5.1f}% {str(d.get('profitFactor', 0)):>6s} {d.get('totalTrades', 0):4d} {d.get('expectancy', 0):+7.3f}R {d.get('maxDrawdownPct', 0):5.1f}%"
    )
    ...
    r = requests.post(
        ...
        json={"style": "auto", "symbol": target_pair}
        if target_pair
        else {"style": "auto"},
        timeout=600,
    )
    ...
    print(
        f"Success: {d.get('success')}  Total pairs: {d.get('totalPairs', 1 if target_pair else 0)}  Errors: {len(d.get('errors', []))}"
    )
    if d.get("errors"):
        for e in d["errors"]:
    ...
    for x in results:
        wf = x.get("wfSplit", {})
        print(
            f"{x['pair']:15s} {x['sqn']:+7.2f} {x['winRate']:5.1f}% {str(x['profitFactor']):>6s} {x['totalTrades']:4d} {x['expectancy']:+7.3f}R {x['maxDrawdownPct']:5.1f}% {wf.get('is_sqn', 0):+7.2f} {wf.get('oos_sqn', 0):+8.2f} {'YES' if wf.get('overfit_flag') else '':>8s}"
        )
    ...
    if not target_pair:
        ...
        for x in results[:5]:
            ...
            if f:
                print(
                    f"  {x['pair']:15s} setups:{f.get('total_setups', 0)} -> fail_score:{f.get('fail_score', 0)} fail_macro:{f.get('fail_macro', 0)} -> taken:{f.get('taken', 0)}"
                )
