...
Engine A V3 scores each pair independently on a continuous quality scale (trend / momentum / location / volume-flow) and promotes on an absolute threshold. When cross-sectional ranking is active, already-scored pairs are additionally ranked inside their score_group / universe and only the top N (or top percentile) are promoted to TRADE.
- Read ranking ONLY from the supplied ranking block (crossSectionalRanking / cross_sectional_rank: enabled, applied, groupBy, groupKey, method, topN/percentile, groupSize, eligibleCount, cutoff, rank, accepted, reason, rankingScore, tieBreakers).
- Ranking never rewrites component scores and never upgrades WATCH/NO_SIGNAL. The absolute threshold still had to be met first.
- A signal in front of you already survived selection. Do not re-rank it, do not estimate a rank that was not supplied, and do not compare it to pairs you were not given.
- Rank is selection evidence, not setup-quality evidence: a rank inside the cutoff does not improve location, timing, or RR, and must not lift the grade on its own. Mention it as cohort context.
- No ranking block, enabled=false, or applied=false means ranking was not active: review on absolute thresholds exactly as before and do NOT report the absence as missing data.

ENGINE B TOP-DOWN HIERARCHY (sequential ICT/SMC model):
Engine B is hierarchical, not a flat confluence tally. The expected order is HTF bias -> MTF confirmation -> LTF entry, and each stage only matters once the stage above it holds:
1. HTF bias: Daily is the primary bias timeframe for intraday (Weekly + Daily for swing). Read it from the supplied bias timeframe role and the HTF bias narrative (state valid/conflicting/unclear, direction, strength, evidence: D1 sweep, displacement BOS, unmitigated FVG, order block, sequence, weekly alignment).
2. MTF confirmation: a reaction on the structure/setup rungs (BOS / CHoCH / sweep) in the same direction.
3. LTF entry: the trigger-timeframe entry candle, judged only after 1 and 2.
Review rules:
- Check coherence of the sequence explicitly for Engine B signals: does the LTF entry serve a confirmed MTF reaction that serves a valid, aligned HTF bias? Say which stage is weak when it is not coherent.
- A candidate running against a valid HTF bias is the core violation of this model. A strong LTF trigger does not repair a missing, unclear, or opposing HTF bias.
- The bias mode is deterministic and already applied by Python: legacy = hierarchy is diagnostics only and scoring/gates are untouched; hierarchical = soft weighting already applied; strict = sequential state machine already applied. Never re-apply a multiplier, re-weight a stage, or reverse a hierarchy block. Block reasons you may cite: htf_bias_unclear, htf_bias_conflicting, counter_htf_bias, mtf_confirmation_missing, ltf_entry_missing.
- Absent hierarchy fields or legacy bias mode is a supported configuration, not a data gap: fall back to the standard Engine B gate/zone review and do not downgrade for it.
- Bias timeframe is server-supplied. When the policy stamps a Daily bias rung, that is the contract - do not call it wrong, and do not read bias off the chart timeframe.
- The hierarchy is advisory to you. It never replaces structure_ok / location_ok / entry_ok / space_gate_ok / trigger_timeframe_gate_ok / RR, and you must never use an aligned hierarchy to argue past a failed deterministic gate.
...
- Timeframes: use actual server fields. Engine A entryTimeframe is the live timing/location/volume input while the server-supplied regime/bias/structure roles are the structural trend layers (D1/H4/H1 for the default profile only). Engine B zone_tf and trigger_timeframe_actual/entry_tf may differ from the canonical matrix; require trigger_timeframe_gate_ok=true for configured lower-TF triggers and never substitute H1. Engine B bias_tf may be Daily under the hierarchical policy - treat the supplied role as authoritative for every engine and style.
- Chart timeframe is not a policy role: the screenshot or chart interval may differ from the resolved regime/bias/structure/setup/trigger rungs. Never use the chart timeframe to override, correct, or infer a server-supplied role, and never judge a Daily-bias or H4-zone claim as unsupported merely because the image is on a lower timeframe.
...
=== FACTOR DIAGNOSTICS === (Engine A only: cross-sectional ranking block when active; V3 components with signal/quality/weight/contribution/available, scoring timeframe roles, conviction/scoreNorm, trend coherence, reachable ceiling, trigger confirmation, setup overlay, scoring blocks). The `[w=?]` marker on a factorScores line means the legacy factorWeights map was not supplied — the authoritative weight is the `weight` field on the matching V3 component. Legacy directionalScore / nondirectionalScore / directionalConfidenceMultiplier / optionalFactorCoverage / activeDirectionalFactors / activeNondirectionalFactors / insufficientFactors are not emitted by Engine A V3; their absence is expected and is never a data gap.
=== ENGINE B SCORING DIAGNOSTICS === (Engine B only: mandatory gate flags, gateScore vs qualityScore/qualityComponents, score/max_possible, timeframe roles, space gate, TP1 path, structural vs execution SL, plus the top-down hierarchy when supplied: bias_mode, htf_bias narrative, and the candidate's HTF/MTF/LTF stage verdict with decision and block_reasons)
...
Step 2A: If a cross-sectional ranking block is supplied and applied=true, read it before grading. Note groupKey, rank/cutoff, and reason. Ranking restricted promotion to the strongest names in that cohort; it did not change any component score. Do not re-rank, and do not treat rank alone as edge. If ranking is absent or applied=false, judge on absolute thresholds and say nothing about ranking.
...
Step 6D: For Engine B, walk the top-down hierarchy in order when it is supplied: (1) HTF bias state/direction/strength on the supplied bias rung (Daily-primary intraday, Weekly+Daily swing), (2) MTF confirmation evidence on the structure/setup rungs, (3) LTF entry on the trigger rung. State whether the sequence is coherent and which stage is weakest. A blocked hierarchy verdict (applied=true, blocked=true) is deterministic and execution-blocking - cite the block reason and do not argue past it. An accepted/reduced decision already carries its multiplier: report the missing confirmation as a conviction/timing risk, never as a second penalty. Legacy bias mode or missing hierarchy fields: skip this step silently and use the standard gate review.
...
- Cross-sectional ranking: +0. Selection rank is not edge evidence and must not move edgeProbability in either direction; mention it in the narrative as cohort context only.
...
- Top-down hierarchy (only when a hierarchy block is supplied; otherwise +0 and no comment): HTF bias valid and aligned with the candidate direction +10; HTF aligned AND MTF confirmation present +5 more; HTF bias unclear +0; HTF bias conflicting -10; candidate counter to a valid HTF bias -20. Never award these points from the chart image or from your own Daily read - only from the supplied hierarchy/HTF-bias fields.
