...
Files: `factor_scoring.py`, `scoring.py`, `indicators.py`, the Engine A sections of `athena.py` (`analyze_pair` level computation only).

- **Buy/sell direction:** `_coherent_trend_score` → `_finalize_coherent_trend_score` → `compute_factor_scores`. Verify direction (`LONG`/`SHORT`/`None`) is decided symmetrically; verify abort paths (`indeterminate_trend`, `min_directional_failed`, `adx_hard_abort`, `adx_missing_both_abort`) fail closed. Crypto VWAP filter `_vwap_direction_filter` is config-gated and crypto-only.
- **ATR SL/TP:** `indicators.calc_levels(price, atr, direction, pair_type, regime_state, style)` — verify `sl`/`tp1`/`tp2`/`rr1`/`rr2` math is correct for BOTH directions; multiplier resolution order `STYLE_ATR_MULTS[style][asset_class]` → `ATR_CLASS[asset_class]` → `SL_ATR_MULT`/`TP1_ATR_MULT`/`TP2_ATR_MULT`; `CALC_LEVELS_REGIME_FACTOR` application. ATR level class via `scoring.get_pair_level_atr_class` (`ENGINE_A_ATR_LEVEL_CLASS_BY_DISPLAY` / `ENGINE_A_ATR_LEVEL_CLASS_BY_SCORE_GROUP`).
- **Overlay interaction:** `indicators.select_overlay_sl` (Engine B `recommended_stop_loss` merge, `ENGINE_A_STRUCTURAL_SL_FLOOR_ATR` floor) and the TP1 clamp against Engine B structural TP in `analyze_pair` — verify the clamp can never invert SL/TP or produce RR worse than reported.
- **Per-group keys to balance-check:** `ENGINE_A_RSI_PERIOD_BY_CLASS`, `ENGINE_A_EMA_PERIODS_BY_CLASS`, `ENGINE_A_MACD_PARAMS_BY_CLASS`, `ENGINE_A_FACTOR_WEIGHTS_BY_CLASS`, `ENGINE_A_SCORE_GROUP_THRESHOLDS`, `ENGINE_A_CONVICTION_FLOOR_BY_CLASS`, `ADX_TREND_MIN_CLASS`.
- **Fix-verification tests (only if a fix is applied):** `tests/test_engine_a_sl_floor.py`, `tests/test_engine_a_level_parity.py`, `tests/test_factor_scoring.py`.
