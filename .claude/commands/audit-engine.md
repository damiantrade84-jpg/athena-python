...
- **Structural SL/TP:** `NakedEngine.analyze_structure_direction` (the `recommended_stop_loss` / `recommended_take_profit` / `tp_source` write site), `_engine_b_structural_target_price` (opposing-zone TP with ATR buffer), `_engine_b_structural_tp_buffer_atr_mult` (`NAKED_ENGINE.structural_tp_buffer_atr_mult`), structural SL from the swing pivot plus `_engine_b_structural_sl_pivot_buffer_atr` (`NAKED_ENGINE.structural_sl_pivot_buffer_atr`, gated by `ENGINE_B_STRUCTURAL_SL_PIVOT_BUFFER_ENABLED`; false reverts to the legacy `NAKED_ENGINE.zone_multipliers[regime].sl` buffer).
- **ATR helper:** `NakedEngine._compute_atr_from_candles` and `resolve_engine_b_tfs` (policy-derived struct/zone/trigger/atr TF roles; `_ENGINE_B_LEGACY_TF_MATRIX` only when the policy mode is non-authoritative), `resolve_engine_b_asset_class` — verify the ATR TF and asset class used for stops match the matrix for every asset class, not just one.
- **RR gate (known past-issue surface — audit hardest here):** `resolve_engine_b_execution_levels` — ATR-vs-structural SL preference, structural TP, RR-tighten before synthesis (`ENGINE_B_PREFER_SL_TIGHTEN_OVER_TP_EXTENSION`), synthetic fallback TP (`ENGINE_B_ALLOW_SYNTHETIC_FALLBACK_RR_TP`, `ENGINE_B_SYNTHETIC_TP_RR_BASIS`, `fallback_rr`), ATR SL clamp (`ENGINE_B_MIN_SL_ATR_DEFAULT` / `ENGINE_B_MAX_SL_ATR_DEFAULT` / `ENGINE_B_ABSOLUTE_MIN_SL_ATR` / `ENGINE_B_ATR_SL_CLAMP_MIN_RESPECTS_STRUCTURAL`), MAX_SL clamp (`ENGINE_B_ENFORCE_MAX_SL_PCT`), `validate_tp_exchange_bounds`, nested `_compute_exec_rr`. Then `calculate_confidence` gate #5: `rr_ok = execution_levels_valid and rr_used_for_gate >= min_rr`. Verify the RR used for the gate is the RR of the levels that would actually execute — no recomputation drift between gate and execution.
