...
def _fetch_page(symbol: str, interval: str, start_dt: datetime,
                end_dt: datetime, api_key: str) -> list:
...
def fetch_twelvedata_bt(display_name: str, days: int = 730) -> tuple[list, list]:
    ...
    log.info(f"[TWELVEDATA] Fetching {display_name} ({td_symbol}) H1 "
    ...
    while page_start < end_dt:
        ...
        try:
            last_dt = datetime.fromisoformat(bars[-1].get("datetime") or bars[-1].get("time") or bars[-1].get("date"))
