...
class DebateCaseResponse(BaseModel):
    ...
    risk_factors: Optional[List[str]] = Field(default=None, description="Risk factors (Bull case)")
    counter_risks: Optional[List[str]] = Field(default=None, description="Counter risks (Bear case)")
...
class JudgeVerdictResponse(BaseModel):
    ...
    score_adjustment: float = Field(ge=-1.0, le=1.0, description="Score adjustment -1.0 to +1.0")
...
class MetaAnalysisResponse(BaseModel):
    ...
    devils_advocate: str = Field(description="Strongest counter-argument to conclusions")
