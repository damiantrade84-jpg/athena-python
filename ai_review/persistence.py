...
def _row_to_response(row: sqlite3.Row) -> dict[str, Any]:
    return {
        ...
        "engine_a_context": json.loads(row["engine_a_snapshot_json"]),
    ...
    *,
    ...
    with sqlite3.connect(path, timeout=15.0) as con:
        ...
        row = con.execute(
            ...
            LIMIT 1
            """,
            (symbol, timeframe, screenshot_hash, f"-{int(window_seconds)} seconds"),
        ).fetchone()
    if not row:
        return None
...
def record_review(
    *,
    ...
    with sqlite3.connect(path, timeout=15.0) as con:
        ...
        con.execute(
            ...
            (
                ...
                json.dumps(engine_a_context, default=str),
                json.dumps(ai_review, default=str),
                json.dumps(concordance, default=str),
                engine_a_context.get("scan_timestamp"),
                engine_a_context.get("chart_captured_at"),
                engine_a_context.get("latest_candle_ts"),
    ...
    return {
        ...
        "timestamps": {
            "scan_timestamp": engine_a_context.get("scan_timestamp"),
            "chart_captured_at": engine_a_context.get("chart_captured_at"),
            "latest_candle_ts": engine_a_context.get("latest_candle_ts"),
        },
