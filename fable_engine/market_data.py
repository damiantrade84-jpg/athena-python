"""Venue-native candle acquisition and strict closed-bar normalization for FABLE.

MT5 rows come from ``fetch_mt5(pair, tf, limit)`` (ISO ``time`` = bar open,
``vol``/``volSource``), Bybit rows from ``_fetch_bybit_klines(symbol, tf, limit)``
(``open_time`` ms, ``confirmed`` flag). Both are reduced to the same ``Candle``
shape and any bar that cannot be proven closed at the evaluation clock is
dropped, so scoring never sees a forming bar.
"""
...
from .models import ROLE_TIMEFRAMES, TIMEFRAME_SECONDS, Candle, MarketSnapshot, utc_iso


class FableMarketDataError(RuntimeError):
    pass
...
def normalize_closed_candles(
    ...
    *,
    ...
    malformed = forming = future = post_as_of = 0
    observed_at = float(observed_at_epoch if observed_at_epoch is not None else now_epoch)
    for raw in rows:
        if not isinstance(raw, dict):
            malformed += 1
            continue
        if raw.get("confirmed") is False or raw.get("closed") is False:
            forming += 1
            continue
        try:
...
class FableMarketDataProvider:
    """Fetch the four FABLE role series from the execution venue for each pair."""
    ...
    def fetch_snapshot(
        ...
        *,
        ...
        errors: list[str] = []
        for timeframe in ROLE_TIMEFRAMES.values():
            try:
                ...
                raw, provider = {"error": True, "detail": str(exc)}, "unavailable"
