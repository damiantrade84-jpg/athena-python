...
Component weights (long side shown; shorts mirror):
  ...
  session     9  killzone timing (London/NY > Asia > off-hours)
  vol         8  ATR-percentile fit band

Grades: A+ >= 85 (requires a sweep or displacement), A >= 75, B >= 65 (watch).
"""
...
def session_weight(ts: float | None = None) -> tuple[float, str]:
    """UTC killzones: London 07-10, NY 12-16, Asia 00-07 (weights 0..1)."""
...
def score(symbol: str, direction: int, *, tide_bias: float, tide_ctx: float,
          pulse_v: float, flow_v: float, pressure_v: float, vwap_rel: float,
          vol_pct: float, events: list[StructureEvent],
          ts: float | None = None) -> ScoreCard:
    """vwap_rel: (price - vwap) / vwap, signed."""
    comp: dict[str, float] = {}
    reasons: list[str] = []
