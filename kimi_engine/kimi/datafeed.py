...
class DataFeed:
    ...
    # ------------------------------------------------------------------
    _MT5_TF = {"5m": "TIMEFRAME_M5", "15m": "TIMEFRAME_M15",
    ...
    def _fetch_mt5_klines(self, symbol: str, tf: str, limit: int) -> CandleSet | None:
        ...
        if resolved is None:
            return None
        try:
            tf_const = getattr(mt5, self._MT5_TF.get(tf, "TIMEFRAME_M15"))
    ...
    def mt5_tick(self, symbol: str) -> dict | None:
        ...
        try:
            ...
            if tk is None or tk.bid <= 0:
                return None
            return {"price": (tk.bid + tk.ask) / 2, "bid": tk.bid, "ask": tk.ask,
                    "source": "mt5"}
        except Exception:  # noqa: BLE001
            return None
    ...
    def _alt_ticker(self, symbol: str) -> dict | None:
        ...
        if y is not None:
            px, chg = y
            return {"price": px, "changePct24h": chg, "quoteVol24h": None,
    ...
    def _yahoo_last_price(self, symbol: str) -> tuple[float, float | None] | None:
        """(last close, hourly change %) from Yahoo — delayed, labeled."""
        ysym = self._YAHOO_MAP.get(symbol)
        if not ysym:
            return None
        try:
