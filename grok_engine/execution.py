"""Fail-closed quote attestation, risk sizing, and broker coordination for GROK."""
...
class GrokExecutionError(RuntimeError):
...
def _asset_limit(mapping: dict[str, Any], asset_type: str) -> float:
    ...
    try:
        ...
        raise GrokExecutionError("EXECUTION_LIMIT_INVALID", detail=asset_type) from exc
    if not math.isfinite(result) or result <= 0:
        raise GrokExecutionError("EXECUTION_LIMIT_INVALID", detail=asset_type)
    return result


class GrokExecutionCoordinator:
    def __init__(
        self,
        *,
        config: GrokConfig,
    ...
    def _assert_mode(self, mode: str, *, confirm_live: bool) -> None:
        ...
        if normalized not in capabilities:
            raise GrokExecutionError("INVALID_EXECUTION_MODE")
        if not bool(capabilities[normalized]["enabled"]):
            raise GrokExecutionError(f"{normalized.upper()}_EXECUTION_DISABLED")
        if normalized == "live":
            if not confirm_live:
                raise GrokExecutionError("LIVE_CONFIRMATION_REQUIRED")
            if os.environ.get("ATHENA_REAL_ORDERS_CONFIRM", "") != "I_UNDERSTAND_REAL_ORDER_RISK":
                raise GrokExecutionError("LIVE_SERVER_CONFIRMATION_MISSING")

    def _closed_bar_stamp(self, signal: dict[str, Any], timeframe: str) -> str | None:
        for gate in signal.get("gates") or []:
            if isinstance(gate, dict) and gate.get("name") == f"{timeframe}_freshness":
                stamp = gate.get("lastClosedAt")
                if stamp:
                    return str(stamp)
        provenance = signal.get("dataProvenance") if isinstance(signal.get("dataProvenance"), dict) else {}
        meta = provenance.get(timeframe)
        if isinstance(meta, dict) and meta.get("lastClosedAt"):
            return str(meta.get("lastClosedAt"))
        return None

    def _live_bar_freshness_gates(self, signal: dict[str, Any]) -> list[dict[str, Any]]:
        gates: list[dict[str, Any]] = []
        now_epoch = float(self.now_fn())
        maximum_age = self.config.scan["maximum_closed_bar_age_buckets"]
        clock_skew = float(self.config.scan["maximum_clock_skew_sec"])
        for timeframe in ("D1", "H1", "M15", "M5"):
            last_iso = self._closed_bar_stamp(signal, timeframe)
            if not last_iso:
                gates.append(
                    {
                        "name": f"{timeframe}_live_freshness",
                        "passed": False,
                        "reason": f"NO_CLOSED_BAR:{timeframe}",
                    }
                )
                continue
            try:
                close_epoch = _parse_iso(last_iso)
            except (TypeError, ValueError):
                gates.append(
                    {
                        "name": f"{timeframe}_live_freshness",
                        "passed": False,
                        "reason": f"CLOSED_BAR_TIMESTAMP_INVALID:{timeframe}",
                        "lastClosedAt": last_iso,
                    }
                )
                continue
            raw_age = now_epoch - close_epoch
    ...
    def preview(self, signal: dict[str, Any]) -> dict[str, Any]:
        ...
        gates.extend(self._live_bar_freshness_gates(signal))
        gates.extend(self._live_session_gates(signal))
    ...
    def _broker_payload(self, signal: dict[str, Any], preview: dict[str, Any]) -> dict[str, Any]:
        ...
        provenance = signal.get("dataProvenance") if isinstance(signal.get("dataProvenance"), dict) else {}
        live_freshness = {
            str(gate.get("name") or "").split("_", 1)[0].upper(): gate
            for gate in (preview.get("gates") or [])
            if isinstance(gate, dict) and str(gate.get("name") or "").endswith("_live_freshness")
        }
        for timeframe in ("D1", "H1", "M15", "M5"):
            gate = live_freshness.get(timeframe) or {}
            source_meta = provenance.get(timeframe)
            fresh = gate.get("passed") is True
            diag: dict[str, Any] = {
                "status": "FRESH" if fresh else str(gate.get("reason") or "UNKNOWN"),
        ...
        return {
            ...
            "grokCandleProvenance": provenance,
            "decision_state": "execute",
            "tier": "TRADE",
            "entryReadiness": "READY",
            "grokSignalId": signal["signalId"],
            "grokContractVersion": signal["contractVersion"],
            "grokQuoteAttestation": preview,
        }

    @staticmethod
    def _positions_list(raw: dict[str, Any]) -> list[dict[str, Any]]:
        if raw.get("error"):
            raise GrokExecutionError("POSITIONS_UNAVAILABLE", detail=str(raw.get("detail") or "unknown"))
        rows = raw.get("positions")
        if not isinstance(rows, list):
            raise GrokExecutionError("POSITIONS_UNAVAILABLE", detail="invalid response")
        return rows

    def _validate_broker_environment(self, mode: str, account: dict[str, Any]) -> None:
        if account.get("error"):
            raise GrokExecutionError("ACCOUNT_UNAVAILABLE", detail=str(account.get("detail") or "unknown"))
        if mode == "demo":
            is_demo = bool(account.get("demo")) or str(account.get("accountEnvironment") or "").lower() == "demo"
            if not is_demo:
                raise GrokExecutionError("DEMO_ACCOUNT_ATTESTATION_FAILED")
        if mode == "live":
            is_real = (
            ...
            if not is_real:
                raise GrokExecutionError("REAL_ACCOUNT_ATTESTATION_FAILED")
    ...
    def _risk_approval(
        ...
        *,
        ...
        if not guardian_ok:
            raise GrokExecutionError("GUARDIAN_REJECTED", detail=guardian_reason)
        ...
        if not levels_unchanged:
            raise GrokExecutionError("IMMUTABLE_LEVELS_MUTATED_BY_RISK_GATE")
        if not approval.approved:
            raise GrokExecutionError("RISK_REJECTED", detail=str(approval.reason))
        ...
        if (
            ...
            raise GrokExecutionError("RISK_APPROVAL_INVALID")
        ...
        if approval.risk_amount > desired_risk > 0:
            ...
            if downsized < minimum or downsized <= 0:
                raise GrokExecutionError("GROK_RISK_BUDGET_BELOW_BROKER_MINIMUM")
    ...
    def execute(
        ...
        *,
        ...
        if not idempotency_key or len(idempotency_key) > 128:
            raise GrokExecutionError("IDEMPOTENCY_KEY_REQUIRED")
        ...
        if not created:
            if reservation.get("signal_id") != signal.get("signalId"):
                raise GrokExecutionError("IDEMPOTENCY_KEY_CONFLICT")
            return {"idempotent": True, **reservation}
        execution_id = reservation["execution_id"]
        try:
            preview = self.preview(signal)
            if not preview.get("executable"):
                raise GrokExecutionError(str(preview.get("error") or "EXECUTION_PREVIEW_REJECTED"))
            if normalized_mode == "paper":
                ...
                result = {
                    "success": True,
                    "mode": "paper",
                    "ticket": "GROK-PAPER-" + execution_id[-12:].upper(),
                ...
                if symbol_info.get("error"):
                    raise GrokExecutionError("SYMBOL_INFO_UNAVAILABLE", detail=str(symbol_info.get("detail") or "unknown"))
                ...
                if not isinstance(result, dict) or not result.get("success"):
                    ...
                    raise GrokExecutionError("BROKER_EXECUTION_REJECTED", detail=str(detail))
                result = {
                    **result,
                    "mode": normalized_mode,
