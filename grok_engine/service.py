"""GROK scan orchestration, account snapshot, and API-facing service methods."""
...
from .execution import GrokExecutionCoordinator
...
class GrokService:
    ...
    def universe(self) -> dict[str, Any]:
        pairs = self._pairs(None, None)
        by_type: Counter[str] = Counter(str(pair.get("type") or "unknown").lower() for pair in pairs)
        return {
            "activePairs": len(pairs),
            "byType": dict(sorted(by_type.items())),
            "symbols": [str(pair.get("display") or pair.get("symbol")) for pair in pairs[:400]],
        }
    ...
    def start_scan(
        ...
        *,
        ...
        if not self.config.enabled:
            raise PermissionError("grok_engine_disabled")
    ...
    def _run_scan(self, scan_id: str, pairs: list[dict[str, Any]]) -> None:
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="GROKPair") as pool:
            ...
            self.log.exception("[GROK] scan failed scan_id=%s", scan_id)
    ...
    def preview_execution(self, signal_id: str) -> dict[str, Any]:
        return self.execution.preview(self._executable_signal(signal_id))
    ...
    def execute_signal(
        ...
        *,
        ...
        return self.execution.execute(
            self._executable_signal(signal_id),
            mode=mode,
            idempotency_key=idempotency_key,
            confirm_live=confirm_live,
        )

    def _executable_signal(self, signal_id: str) -> dict[str, Any]:
        from .execution import GrokExecutionError

        if not self.config.enabled:
            raise GrokExecutionError("GROK_ENGINE_DISABLED")
        signal = self.signal(signal_id)
        pair_key = str(signal.get("pair") or signal.get("symbol") or "")
        if not self._pairs(None, {pair_key} if pair_key else None):
    ...
    def replay_pair(self, symbol: str, *, bars: int | None = None) -> dict[str, Any]:
        if not bool(self.config.replay.get("enabled", True)):
            raise PermissionError("grok_replay_disabled")
        ...
        limits = {
            ...
            "D1": min(1000, max(int(self.config.scan["bars"]["D1"]), math.ceil(requested / 288) + 40)),
        }
