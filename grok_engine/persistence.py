"""SQLite persistence and idempotency ledger for GROK."""
...
class GrokRepository:
    ...
    def migrate(self) -> None:
        with self._migration_lock, self._connect() as con:
            con.executescript(
                """
                CREATE TABLE IF NOT EXISTS grok_scans (
                ...
                CREATE TABLE IF NOT EXISTS grok_signals (
                    ...
                    FOREIGN KEY(scan_id) REFERENCES grok_scans(scan_id)
                );
                CREATE INDEX IF NOT EXISTS idx_grok_signals_scan_score
                    ON grok_signals(scan_id, decision, score DESC);

                CREATE TABLE IF NOT EXISTS grok_executions (
                    ...
                    FOREIGN KEY(signal_id) REFERENCES grok_signals(signal_id)
                );
                CREATE INDEX IF NOT EXISTS idx_grok_executions_signal
                    ON grok_executions(signal_id, requested_at DESC);
                """
            )

    def create_scan(self, request_payload: dict[str, Any]) -> str:
        scan_id = "grokscan_" + uuid.uuid4().hex
        with self._connect() as con:
            con.execute(
                "INSERT INTO grok_scans(scan_id,started_at,status,request_json,summary_json) VALUES(?,?,?,?,?)",
    ...
    def complete_scan(self, scan_id: str, status: str, summary: dict[str, Any]) -> None:
        with self._connect() as con:
            con.execute(
                "UPDATE grok_scans SET completed_at=?, status=?, summary_json=? WHERE scan_id=?",
    ...
    def latest_scan(self) -> dict[str, Any] | None:
        with self._connect() as con:
            row = con.execute("SELECT * FROM grok_scans ORDER BY started_at DESC LIMIT 1").fetchone()
    ...
    def latest_completed_scan_id(self) -> str | None:
        with self._connect() as con:
            row = con.execute(
                "SELECT scan_id FROM grok_scans WHERE status='COMPLETED' ORDER BY completed_at DESC LIMIT 1"
            ).fetchone()
    ...
    def list_signals(
        ...
        *,
        ...
        query = f"SELECT payload_json FROM grok_signals WHERE {' AND '.join(clauses)} ORDER BY score DESC, pair ASC LIMIT ?"
        with self._connect() as con:
    ...
    def get_signal(self, signal_id: str) -> dict[str, Any] | None:
        with self._connect() as con:
            row = con.execute("SELECT payload_json FROM grok_signals WHERE signal_id=?", (signal_id,)).fetchone()
    ...
    def reserve_execution(
        ...
        *,
        ...
        execution_id = "grokexec_" + uuid.uuid4().hex
        with self._connect() as con:
            con.execute("BEGIN IMMEDIATE")
            existing_key = con.execute(
                "SELECT * FROM grok_executions WHERE idempotency_key=?", (idempotency_key,)
            ).fetchone()
            if existing_key:
                return self._execution_dict(existing_key), False
            active = con.execute(
                """
                SELECT * FROM grok_executions
            ...
            row = con.execute("SELECT * FROM grok_executions WHERE execution_id=?", (execution_id,)).fetchone()
    ...
    def complete_execution(self, execution_id: str, status: str, result: dict[str, Any]) -> dict[str, Any]:
        with self._connect() as con:
            con.execute(
                "UPDATE grok_executions SET status=?, completed_at=?, result_json=? WHERE execution_id=?",
    ...
    def list_executions(self, *, limit: int = 100) -> list[dict[str, Any]]:
        with self._connect() as con:
            rows = con.execute(
                "SELECT * FROM grok_executions ORDER BY requested_at DESC LIMIT ?",
